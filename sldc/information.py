...
class WorkflowInformation(object):
    ...
    def __init__(self, polygons, labels, timing, **kwargs):
        ...
        **kwargs: 
        ...
        data_fields = set(kwargs.keys()).intersection(self.__dict__.keys())
...
class ChainInformation(object):
    ...
    def __iter__(self):
        for label, workflow_info in self._infos.items():
            yield label, workflow_info

    @property
    def polygons(self):
        """Return all the found polygons"""
        return np.array([p for info in self._infos.values() for p in info.polygons])

    @property
    def labels(self):
        """Return all the found labels"""
        return np.array([l for info in self._infos.values() for l in info.labels])
