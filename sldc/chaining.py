...
class WorkflowExecutor(Loggable):
    ...
    def __init__(self, workflow, translate=True, logger=SilentLogger()):
    ...
    def execute(self, windows):
        ...
        for i, window in enumerate(windows):
            self.logger.info("WorkflowExecutor : process image {}/{}.".format(i + 1, window_count))
            returned_info = self._workflow.process(window)
            if self._do_translate:
                self._translate(window, returned_info)
